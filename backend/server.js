...
app.post('/api/export-markdown', (req, res) => {
  try {
    const { studyGuide, language = 'en' } = req.body;
    if (!studyGuide) {
      return res.status(400).json({ error: 'No study guide data provided.' });
    }
    ...
    let md = `# ${safeStr(studyGuide.title)}\n\n`;
    md += `**${zh ? '经文' : 'Passage'}:** ${safeStr(studyGuide.passage)}  \n`;
    md += `**${zh ? '神学立场' : 'Theology'}:** ${safeStr(studyGuide.theology)}\n\n`;
    md += `---\n\n`;
    ...
    if (studyGuide.overview) {
      ...
      if (studyGuide.overview.introduction) {
        md += `### ${zh ? '简介' : 'Introduction'}\n\n${safeStr(studyGuide.overview.introduction)}\n\n`;
      }
      if (studyGuide.overview.historicalContext) {
        md += `### ${zh ? '历史背景' : 'Historical Context'}\n\n${safeStr(studyGuide.overview.historicalContext)}\n\n`;
      }
      if (studyGuide.overview.literaryContext) {
        md += `### ${zh ? '文学背景' : 'Literary Context'}\n\n${safeStr(studyGuide.overview.literaryContext)}\n\n`;
      }
    }

    // Exegesis
    const exegesis = safeArr(studyGuide.exegesis);
    if (exegesis.length > 0) {
      md += `## ${zh ? '逐节解经' : 'Verse-by-Verse Exegesis'}\n\n`;
      for (const verse of exegesis) {
        md += `### ${safeStr(verse.verse)}\n\n`;
        if (verse.text) md += `> *${safeStr(verse.text)}*\n\n`;
        md += `${safeStr(verse.explanation)}\n\n`;
        const insights = safeArr(verse.keyInsights);
        if (insights.length > 0) {
          md += `**${zh ? '关键洞察' : 'Key Insights'}:**\n`;
          for (const insight of insights) md += `- ${safeStr(insight)}\n`;
          md += `\n`;
        }
        const refs = safeArr(verse.crossReferences);
        if (refs.length > 0) {
          md += `**${zh ? '参考经文' : 'Cross References'}:** ${refs.map(safeStr).join(', ')}\n\n`;
        }
        md += `---\n\n`;
      }
    }
    ...
    const questions = safeArr(studyGuide.discussionQuestions);
    if (questions.length > 0) {
      md += `## ${zh ? '讨论问题' : 'Discussion Questions'}\n\n`;
      questions.forEach((q, i) => { md += `${i + 1}. ${safeStr(q)}\n`; });
      md += `\n`;
    }
    ...
    if (studyGuide.lifeApplication) {
      ...
      const apps = safeArr(studyGuide.lifeApplication.practicalApplications);
      if (apps.length > 0) {
        md += `### ${zh ? '实际应用' : 'Practical Applications'}\n\n`;
        for (const a of apps) md += `- ${safeStr(a)}\n`;
        md += `\n`;
      }
      const reflections = safeArr(studyGuide.lifeApplication.reflectionPoints);
      if (reflections.length > 0) {
        md += `### ${zh ? '反思要点' : 'Reflection Points'}\n\n`;
        for (const r of reflections) md += `- ${safeStr(r)}\n`;
        md += `\n`;
      }
      const steps = safeArr(studyGuide.lifeApplication.actionSteps);
      if (steps.length > 0) {
        md += `### ${zh ? '行动步骤' : 'Action Steps'}\n\n`;
        for (const s of steps) md += `- ${safeStr(s)}\n`;
        md += `\n`;
      }
    }
    ...
    if (studyGuide.additionalResources) {
      md += `## ${zh ? '其他资源' : 'Additional Resources'}\n\n`;
      const memVerses = safeArr(studyGuide.additionalResources.memoryVerses);
      if (memVerses.length > 0) {
        md += `**${zh ? '背诵经文' : 'Memory Verses'}:** ${memVerses.join(', ')}\n\n`;
      }
      const prayers = safeArr(studyGuide.additionalResources.prayerPoints);
      if (prayers.length > 0) {
        md += `**${zh ? '祷告要点' : 'Prayer Points'}:**\n`;
        for (const p of prayers) md += `- ${safeStr(p)}\n`;
        md += `\n`;
      }
    }

    // Commentaries
    const commentaries = safeArr(studyGuide.commentariesUsed);
    if (commentaries.length > 0) {
      md += `## ${zh ? '使用的注释书' : 'Commentaries Used'}\n\n`;
      for (const c of commentaries) {
        md += `- ${safeStr(c.citation)} **${safeStr(c.name)}** by ${safeStr(c.author)}`;
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="study-guide.md"');
    res.send(md);
  } catch (error) {
