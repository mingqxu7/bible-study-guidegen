// Comprehensive Bible verse bounds data
// Contains the number of chapters for each book and the number of verses for each chapter

export const bibleBounds = {
  // Old Testament
  "gen": {
    chapters: 50,
    verses: {
  ...
  "neh": {
    chapters: 13,
    verses: { 1: 11, 2: 20, 3: 32, 4: 23, 5: 19, 6: 19, 7: 73, 8: 18, 9: 38, 10: 39, 11: 36, 12: 47, 13: 31 }
  },
  "est": {
    chapters: 10,
  ...
  "ecc": {
    chapters: 12,
    verses: { 1: 18, 2: 26, 3: 22, 4: 16, 5: 20, 6: 12, 7: 29, 8: 17, 9: 18, 10: 20, 11: 10, 12: 14 }
  },
  "sng": {
    chapters: 8,
  ...
  "dan": {
    chapters: 12,
    verses: { 1: 21, 2: 49, 3: 30, 4: 37, 5: 31, 6: 28, 7: 28, 8: 27, 9: 27, 10: 21, 11: 45, 12: 13 }
  },
  "hos": {
    chapters: 14,
    verses: { 1: 11, 2: 23, 3: 5, 4: 19, 5: 15, 6: 11, 7: 16, 8: 14, 9: 17, 10: 15, 11: 12, 12: 14, 13: 16, 14: 9 }
  },
  "joe": {
    chapters: 3,
  ...
  "zec": {
    chapters: 14,
    verses: { 1: 21, 2: 13, 3: 10, 4: 14, 5: 11, 6: 15, 7: 14, 8: 23, 9: 17, 10: 12, 11: 17, 12: 14, 13: 9, 14: 21 }
  },
  "mal": {
    chapters: 4,
  ...
  "rom": {
    chapters: 16,
    verses: { 1: 32, 2: 29, 3: 31, 4: 25, 5: 21, 6: 23, 7: 25, 8: 39, 9: 33, 10: 21, 11: 36, 12: 21, 13: 14, 14: 23, 15: 33, 16: 27 }
  },
  "1co": {
    chapters: 16,
    verses: { 1: 31, 2: 16, 3: 23, 4: 21, 5: 13, 6: 20, 7: 40, 8: 13, 9: 27, 10: 33, 11: 34, 12: 31, 13: 13, 14: 40, 15: 58, 16: 24 }
  },
  "2co": {
    chapters: 13,
    verses: { 1: 24, 2: 17, 3: 18, 4: 18, 5: 21, 6: 18, 7: 16, 8: 24, 9: 15, 10: 18, 11: 33, 12: 21, 13: 14 }
  },
  "gal": {
    chapters: 6,
  ...
  "heb": {
    chapters: 13,
    verses: { 1: 14, 2: 18, 3: 19, 4: 16, 5: 14, 6: 20, 7: 28, 8: 13, 9: 28, 10: 39, 11: 40, 12: 29, 13: 25 }
  },
  "jam": {
    chapters: 5,
...
// Helper function to get book bounds
export function getBookBounds(bookCode) {
  return bibleBounds[bookCode] || null;
}

// Helper function to validate chapter number
export function isValidChapter(bookCode, chapter) {
  const bounds = getBookBounds(bookCode);
  if (!bounds) return false;
  return chapter >= 1 && chapter <= bounds.chapters;
}

// Helper function to validate verse number
export function isValidVerse(bookCode, chapter, verse) {
  const bounds = getBookBounds(bookCode);
  if (!bounds) return false;
  if (!isValidChapter(bookCode, chapter)) return false;
  const maxVerse = bounds.verses[chapter];
  return verse >= 1 && verse <= maxVerse;
}

// Helper function to get max verse count for a chapter
export function getMaxVerse(bookCode, chapter) {
  const bounds = getBookBounds(bookCode);
  if (!bounds || !isValidChapter(bookCode, chapter)) return 0;
  return bounds.verses[chapter];
}